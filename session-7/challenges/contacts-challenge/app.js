...
inputField.addEventListener("keyup", function (event) {
  ...
  for (let name of names) {
    ...
    if (name.toLowerCase().startsWith(input)) {
      console.log(name);
      items.forEach((item) => {
        if (item.textContent.toLowerCase().startsWith(input) === false) {
...
// Filter through each elemtn in the array
// Search for letters in our input eventlistener
// Edit html to only show those contents
